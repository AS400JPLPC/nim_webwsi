...
function isDateFmt(str, options) {
  ...
  var locale = options.format;
	...
	$.each(fmtdate,function(propx){  
          if(propx == locale) { if( fmtdate[locale].test(str) ) _err = true ;}  
    });
	    ...
	    if(locale == 'mm/dd/yyyy' ||locale == 'mm-dd-yyyy' ) {   
	    ...
	    if( locale == 'yyyy/mm/dd' || locale == 'yyyy-mm-dd' ) {
		 		var year  = str.substr(0,4);   
